 ...
 */
/*
 ** Code for processing tables in the LMNO parser generator.
 */
...
void Strsafe_init()
{
	...
	if( x1a )
	{
		x1a->size = 1024;
		x1a->count = 0;
		x1a->tbl = (x1node*)malloc((sizeof(x1node) + sizeof(x1node*))*1024 );
		if( x1a->tbl==0 )
		{
			free(x1a);
			x1a = 0;
...
int Symbol_insert(struct symbol *data, char *key)
{
	...
	if( x2a->count>=x2a->size )
	{
		...
		array.tbl = (x2node*)malloc((sizeof(x2node) + sizeof(x2node*))*size );
		if( array.tbl==0 )
			return 0;  /* Fail due to malloc failure */
...
int State_insert(struct state *data, struct config *key)
{
	...
	if( x3a->count>=x3a->size )
	{
		...
		array.tbl = (x3node*)malloc((sizeof(x3node) + sizeof(x3node*))*size );
		if( array.tbl==0 )
			return 0;  /* Fail due to malloc failure */
...
int Configtable_insert(struct config *data)
{
	...
	if( x4a->count>=x4a->size )
	{
		...
		array.tbl = (x4node*)malloc((sizeof(x4node) + sizeof(x4node*))*size );
		if( array.tbl==0 )
			return 0;  /* Fail due to malloc failure */
